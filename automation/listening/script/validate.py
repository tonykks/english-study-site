...
from automation.listening.script.caption_restore import is_complete_sentence, validate_missing_short_boundaries, validate_no_artificial_sentence_breaks, validate_punctuation_anomalies, validate_run_on_sentences
...
def validate_no_transcript_anomalies(segments: list[Segment]) -> ValidationResult:
    ...
    punct = validate_punctuation_anomalies(full)
    if not punct.ok:
        return punct

    missing = validate_missing_short_boundaries(full)
    if not missing.ok:
        return missing
